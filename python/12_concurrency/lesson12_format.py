...
def calculate_sum_parallel(start: int, end: int) -> int:
    ...
    chunks = [
        (int(start + i * chunk_size), int(min(start + (i + 1) * chunk_size - 1, end)))
        for i in range(chunk_num)
    ]
    print(chunks)
    with Pool(chunk_num) as pool:
        chunk_result = pool.starmap(
            calculate_sum, chunks
        )  # starmap works with multy args (tuples), map with 1
...
def calculate_sum_parallel_thread(start: int, end: int) -> int:
    ...
    with ThreadPoolExecutor(max_workers=cpu_count()) as executor:
        chunk_result = executor.map(
            lambda args: calculate_sum(*args), chunks
        )  # lambda to unpack tuple into 2 args for calc_sum
    return sum(chunk_result)
...
async def sleep_time_async():
    sleep_time = random.randrange(10)
    print(f"Sleep time is {sleep_time:} seconds.")
...
if __name__ == "__main__":
    ...
    # print(calculate_sum(1, 2**30))
    duration = time.perf_counter() - start
    # print(f'Execution time of single: {duration:.2f}')
    start = time.perf_counter()
    # print(calculate_sum_parallel(1, 2**30))
    duration = time.perf_counter() - start
    # print(f'Execution time of parallel: {duration:.2f}')
    start = time.perf_counter()
    # print(calculate_sum_parallel_thread(1, 2**30))
    duration = time.perf_counter() - start
    # print(f'Execution time of parallel thread: {duration:.2f}')
    ...
    print(f"Elapsed time for thread: {elapsed_time:.4f} seconds")

    total = sum(workload)
    max_wl = max(workload)
    print(f"Total workload time is  {total:} seconds.")
    print(f"Max workload time is  {max_wl:} seconds.")
    print(workload)
    print(len(workload))
    ...
    end_time = time.perf_counter()
    elapsed_time = end_time - start_time
    print(f"Elapsed time for async: {elapsed_time:.4f} seconds")

    print(as_result)
    print(len(as_result))
    total_as = sum(as_result)
    max_as = max(as_result)
    print(f"Total workload time is  {total_as:} seconds.")
    print(f"Max workload time is  {max_as:} seconds.")

...
def sleepy():
    ...
    logger.debug(
        f'Function "{sleepy.__name__}" will sleep for {sleep_val} sec during execution'
    )
...
def sleep_n_times():
    ...
    with ThreadPoolExecutor(max_workers=number_of_threads) as executor:
        ...
        for f in concurrent.futures.as_completed(futures):
            ...
            counter += 1
            logger.debug(
                f'Function "{sleepy.__name__}" execution  #{counter} took {f.result()} sec'
            )
...
if __name__ == "__main__":
    ...
    logger.info(
        f'One-time execution of function "{sleepy.__name__}" took {sleepy_duration:.3f} sec'
    )
    logger.debug(
